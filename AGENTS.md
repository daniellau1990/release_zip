...
## 项目概述
Python 库 + CLI 工具，用于压缩/解压 ZIP、RAR、7z 格式。
...
- **中文编码**：RAR 需处理 CP936，ZIP 用 UTF-8
- **压缩策略**：优先用 .zip 或 .7z，RAR 压缩是可选降级
...
> `.venv/` 已加入 `.gitignore`，不要提交到仓库。

## 三层防御方案（参考 Guardrail 方案设计）

### Tier 1: AGENTS.md 结构强化（预防层）
- 工作流和合规清单放在文件顶部，强制 AI 在写代码前阅读
- `QUICK COMPLIANCE CHECK` 自查清单
- `opencode.json` 中 `"instructions": ["AGENTS.md"]` 确保每次会话加载
...
**A. OpenCode 插件（自动）**
位于 `.opencode/plugins/workflow-guard.js`，通过 `tool.execute.before` 钩子在每次 `edit`/`write`/`apply_patch` 前自动运行检查：
- 无计划文件 → 抛出错误，阻止操作
- AI 无法绕过，无需人工干预

**B. workflow_check.py（手动/底层）**
位于 `.opencode/hooks/workflow_check.py`，可手动调用：
- 检查今日 `docs/plans/YYYY-MM-DD-*.md` 是否存在
- 白名单路径（`docs/`、`AGENTS.md`、`Version.md`、`.githooks/` 等）永远放行
- 源代码目录（`zip_rar_tool/`、`tests/`）必须有对应计划文件
- 退出码：0=放行  1=警告  2=BLOCK
...
2. 验证 AGENTS.md 声明的文件结构完整
3. 运行全部测试（pytest），失败则阻止 commit
...
### 0. 加载上下文 → `openspec/specs/`
- 每次任务开始前，先读取 `openspec/specs/` 获取项目背景
- spec 是需求的地基，不读 spec 不动手

### 1. 需求澄清 → `ahthropics-skill-brainstorming`
- 与用户 brainstorm 确认真实需求，理解约束和优先级
- 不假设需求，不默默决策
...
> **WARNING: 即使 docs/plans/ 有计划文件，也不得跳过 Steps 2-3 (propose -> review)。**
